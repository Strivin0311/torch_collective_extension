...
ExtNCCLComm::ExtNCCLComm(ncclComm_t ncclComm) : ncclComm_(ncclComm) {}

ExtNCCLComm::~ExtNCCLComm() noexcept {
...
ExtNCCLComm::ExtNCCLComm(ExtNCCLComm&& other) {
...
ncclUniqueId ExtNCCLComm::getNcclId() {
    return ncclId_;
}

std::shared_ptr<ExtNCCLComm> ExtNCCLComm::create(
    int numRanks,
    int rank,
    ncclUniqueId commId,
    at::DeviceIndex deviceIndex) {
    at::cuda::OptionalCUDAGuard gpuGuard(deviceIndex);
    auto comm = std::make_shared<ExtNCCLComm>();
    C10D_NCCL_CHECK(
...
ncclComm_t ExtNCCLComm::getNcclComm() {
    LockType lock(mutex_);
    if (aborted_) {
...
void ExtNCCLComm::waitReady(bool longInterval) {
    LockType lock(mutex_);
    if (aborted_)
    return;
...
std::optional<std::string> ExtNCCLComm::getNcclCommFailureReason() const {
    LockType lock(mutex_);
...
std::shared_ptr<ExtNCCLComm> ExtNCCLComm::split(
    ExtNCCLComm* source,
    int color_id,
    int rank,
    ncclConfig_t& config,
...
void ExtNCCLComm::abort(std::optional<std::string> commFailureReason) {
    LockType lock(mutex_);
    at::cuda::OptionalCUDAGuard gpuGuard(deviceIndex_);
...
bool ExtNCCLComm::isInitialized() const {
    LockType lock(mutex_);
    return initialized_;
}

bool ExtNCCLComm::isAborted() const {
    LockType lock(mutex_);
    return aborted_;
}

uint64_t ExtNCCLComm::getCommSplitCounter() const {
    return ncclCommSplitCounter_;
}

ncclResult_t ExtNCCLComm::checkForNcclError() {
    LockType lock(mutex_);
#ifdef ENABLE_NCCL_ERROR_CHECKING
    if (ncclAsyncErr_ != ncclSuccess) {
...
ncclResult_t ExtNCCLComm::registerSegment(void* ptr, size_t size) {
    LockType lock(mutex_);
#ifdef NCCL_HAS_COMM_REGISTER
...
ncclResult_t ExtNCCLComm::deregisterSegment(void* ptr) {
    LockType lock(mutex_);
#ifdef NCCL_HAS_COMM_REGISTER
    TORCH_CHECK(
...
std::string ExtNCCLComm::repr() const {
    return c10::str((void*)ncclComm_);
}

#if (defined(IS_NCCLX) || defined(USE_ROCM)) && defined(NCCL_COMM_DUMP)
std::unordered_map<std::string, std::string> ExtNCCLComm::ncclCommDump() {
